...
int main() {
    ...
    serverAddr.sin_addr.s_addr = inet_addr("127.0.0.1"); // checking for loopback to work, replace it with htonl(INADDR_ANY)
    serverAddr.sin_port = htons(12345);

    // binding the socket to the server address
    int status = bind(servSocket, (struct sockaddr*)&serverAddr, sizeof(serverAddr));
    if (status != 0) {
    ...
    clientConn = accept(servSocket, (struct sockaddr*)&clientAddr, &len);
    if (clientConn < 0) {
