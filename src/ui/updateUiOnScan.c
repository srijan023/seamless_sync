...
static void update_ui_on_scan(gpointer user_data) {
  ...
  GtkWidget *vertical_box_for_client_and_server =
      create_vertical_box(10, 0, 0, 0, 0);
  ...
  g_signal_connect(sidebar, "row-activated",
                   G_CALLBACK(on_device_list_activated),
                   vertical_box_for_client_and_server);

  gtk_scrolled_window_set_child(GTK_SCROLLED_WINDOW(scrolled_window), sidebar);
  gtk_box_append(GTK_BOX(vertical_box_for_client_and_server), frame);
  gtk_window_set_child(data->window, vertical_box_for_client_and_server);
